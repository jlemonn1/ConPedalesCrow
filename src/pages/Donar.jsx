...
export default function Donar() {
  ...
  const [touched, setTouched] = useState({ amount: false, name: false, email: false, comment: false });
  ...
  const commentError = touched.comment && formData.comment.length > 255;
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setTouched({ amount: true, name: true, email: true, comment: true });

    if (!finalAmount || finalAmount < 0.5) return;
    if (!formData.name.trim()) return;
    if (emailRequired && !formData.email.trim()) return;
    if (formData.comment.length > 255) return;
          ...
          <div className="donar-card">
            ...
            <form className="donar-form" onSubmit={handleSubmit} noValidate>
              ...
              <div className="donar-section">
                ...
                <div className={`donar-input-group textarea-group ${commentError ? 'has-error' : ''}`}>
                  <svg className="input-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor">
